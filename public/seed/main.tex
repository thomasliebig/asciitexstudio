...
Long tables wrap cell content instead of clipping it. They use the same table counter
unless numbered=false is set.

\label{tab:long}
\begin{longasciitable}[width=\textwidth,align=ll,header=true,style=single,borders=outer,caption="Wrapped long table"]
Column & Description
longasciitable & Long cell content is formatted through the paragraph wrapper, so prose can span several table lines while keeping the column grid stable.
borders=outer & Draws only the outside border and avoids internal vertical grid lines for a cleaner reading table.
\end{longasciitable}

\begin{longasciitable}[width=\textwidth,numbered=false,style=dashed,borders=rows,caption="Unnumbered dashed long table"]
Border option & Effect
none & Omits table border lines entirely while preserving column spacing.
rows & Draws horizontal lines between every table row. Use grid for both row and column lines.
style=dashed & Uses dashed horizontal and vertical border glyphs.
Manual linebreaks & Use \\ inside a cell for a line break, and \\ \\ for an empty line.
\end{longasciitable}

Border styles are split into glyph style and placement: style=single, double, heavy,
rounded, dashed, or ascii; borders=all, grid, outer, none, horizontal, vertical,
top,bottom,left,right,header,rows,cols, or combinations such as top,bottom.
...
Boxes may contain other AsciiTeX environments. Use place=h for here, place=t for
top, place=b for bottom, and place=inline to disable floating.
\begin{box}[width=\textwidth,style=rounded,place=h,title="Nested syntax"]
Manual line breaks work in prose: first line\newline second line.
\begin{itemize}
\item Lists work inside boxes.
\item Equations and bib entries are rendered as nested blocks.
\end{itemize}
\begin{equation*}
  E = mc^2
\end{equation*}
...
\begin{longasciitable}[width=\columnwidth,place=b,header=true,borders=rows,caption="Bottom table float"]
Option & Effect
place=t & Float to the next column top.
place=b & Float to the current column bottom.
place=h & Keep near the source position.
\end{longasciitable}
...
without a reference number; if no optional file list is given, it uses the bibliography
files from this document. You may override that with \bibentry[refs.bib]{knuth1981}.

\bibentry{knuth1981}

Bibentry can also be used inside lists.
\begin{itemize}
\item Inline reference entry:
\bibentry{knuth1981}
\item Explicit bibliography file:
\bibentry[refs.bib]{knuth1981}
\end{itemize}
